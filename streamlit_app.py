...
# Google Drive 파일 ID
file_id = '1blpN6V_2DGFEgGLR8hn-2PLEgDhqpwCe'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/px1Nd6L/kad4gm7m-I8-UZw-JWLd-JN8e.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=_j-U_ugWreM", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 누덕누덕스타카토야.")
